...
$slim_app->get('/loadUsersCombo','loadUsersCombo');
$slim_app->post('/newUser','newUser');
$slim_app->post('/editUser','editUser');
$slim_app->post('/updaUser','updaUser');
$slim_app->get('/loadEditUser/:eid','loadEditUser');
$slim_app->delete('/deleteUser/:did','deleteUser');
//***lista de canes
$slim_app->get('/loadCan','loadCan');
//***************combo cascada
$slim_app->get('/listdepartamentos','listdepartamentos');
$slim_app->get('/listdeprovincias/:eid','listdeprovincias');
...
function loadCan() {
	$sql = "SELECT `ANIMAL_CHIP_ID`,`ANIMAL_NOMBRE`,`ANIMAL_ESPECIE`,`ANIMAL_RAZA`,`ANIMAL_SEXO`,`ANIMAL_COLOR`,`ANIMAL_PELAJE`,`ANIMAL_DESCRIPCION`,ANIMAL_NACIMIENTO FROM `zn_animal` 
where `ANIMAL_ESTADO`='ACTIVO' ORDER BY `ANIMAL_ID`ASC";
	try {
		$db = getDB('mysql');
		$stmt = $db->query($sql);  
...
function listdepartamentos() {
	$sql = "SELECT id,ciudad FROM `departamento` WHERE 1";
	try {
		$db = getDB('mysql');
		$stmt = $db->query($sql);  
		$users = $stmt->fetchAll(PDO::FETCH_OBJ);
...
//lista de provincias	
/*function listdeprovincias($did) {
	$sql = "SELECT id,nombre FROM `provincia` WHERE id_dep=:id_depart";
	try {
		$db = getDB('mysql');
		$stmt = $db->query($sql);  
		$users = $stmt->fetchAll(PDO::FETCH_OBJ);
...
function listdeprovincias($eid) {  
	$sql = "SELECT id,nombre FROM `provincia` WHERE id_dep=:id";
	try {		
		$db = getDB('mysql');		
		$stmt = $db->prepare($sql);
...
function loadUsersCombo() {
	$sql = "SELECT uid as id,name FROM customers_auth ORDER BY uid asc";
	try {
		$db = getDB('mysql');
		$stmt = $db->query($sql);  
		$users = $stmt->fetchAll(PDO::FETCH_OBJ);
...
function loadEditUser($eid) {  
	$sql = "SELECT uid as id,name,email, phone as mobile FROM customers_auth WHERE uid=:id";
	try {		
		$db = getDB('mysql');		
		$stmt = $db->prepare($sql);
		$stmt->bindValue(':id', $eid);		
