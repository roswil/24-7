...
$slim_app->get('/listaAccesos','listaAccesos');
...
function listaAccesos() {
	$sql = "SELECT ac.id_acceso,ac.id_opcion,op.opcion as opcion,ac.id_rol,ro.rol as rol,ac._registrado as _registrado ,ac._modificado as _modificado,ac._id_usuario,ac._estado
			FROM _bp_accesos ac
			INNER JOIN _bp_opciones op ON op.id_opcion = ac.id_opcion 
			INNER JOIN _bp_roles ro ON ro.id_rol = ac.id_rol
			WHERE ac._estado = 'A'";
	try {
		$db = getDB('mysql');
		$stmt = $db->query($sql);  
