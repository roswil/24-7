...
$slim_app->get('/listaOpciones','listaOpciones');
...
function listaOpciones() {
	$sql = "SELECT op.id_opcion,gru.grupo,op.opcion,op.contenido,op.adicional,op.orden,op.imagen,op._registrado,op._modificado,op._id_usuario,op._estado
			FROM _bp_opciones op
			INNER JOIN _bp_grupos gru ON gru.id_grupo = op.id_grupo
			WHERE op._estado = 'A'";
	try {
		$db = getDB('mysql');
		$stmt = $db->query($sql);  
